      ...
      "messages": [
        {
          ...
          "content": [
            // {"type":"text", "text": "你是一名宠物护理专家，能够准确的根据图片识别出图片中的宠物类型、宠物种类、宠物的长、宠物的身高、宠物的体重、宠物的毛发长度。"},
            {
              "type": "text",
              "text":
                  '''你是一名经验丰富的宠物护理专家，擅长从宠物图片中提取专业信息。请根据提供的宠物图片，识别出以下内容，并以指定的 JSON 格式返回：
宠物类型（如：猫、狗、兔子等）
宠物品种（如：金毛寻回犬、布偶猫等）
宠物身长（单位：厘米）
宠物身高（单位：厘米）
...
''',
    ...
    debugPrint(
      "request url : $url , request headers : $headers , request body : $body",
    );
  ...
  static Future<bool> uploadEsResult(
    ...
    Map<String, dynamic> body = {"pet_info": petInfo, "image_path": imagePath};
    try {
      var response = await http.post(
        esUrl,
        headers: headers,
  ...
  static Future<void> launchrURL(String url) async {
    debugPrint("launchrURL : $url");
    final Uri dingTalkUrl = _formatDingTalkUrl(url);
    if (await canLaunchUrl(dingTalkUrl)) {
      await launchUrl(dingTalkUrl);
...
Uri _formatDingTalkUrl(String url) {
  //  final Uri dingTalkUrl = Uri.parse('dingtalk://dingtalkclient/page/link?url=https%3A%2F%2Falidocs.dingtalk.com%2Fnotable%2Fshare%2Fform%2Fv011wvqrebAwQ2rvnak_modftrA_iacJAgh');
  return Uri.parse('dingtalk://dingtalkclient/page/link?url=$url');
}
